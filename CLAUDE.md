...
1. **修正前**: 該当ページのHTMLを `curl` で取得する、または既存スクリーンショットがあればそれを参照して現状の構造・余白・配置を把握する
2. **修正後**: ローカルで `npm run dev` を起動して `curl http://localhost:3000/...` で HTML を取得、または WebFetch でレンダリング後の構造を確認する
3. 確認した内容（何が変わったか・期待通りか）をユーザーに報告してから完了とする

### やらないこと

- 本番デプロイの完了を待ってからの確認は不要（時間がかかるため）
- 「コードを修正してコミットしたら終わり」にしない。レビューの目を必ず通す
- WebFetch だけで「OK」と判断しない（WebFetch は構造概要しか返さない）。ピクセル単位の余白・サイズが論点なら、HTMLと該当CSSクラスを直接確認する
